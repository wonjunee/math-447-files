...
function movie=oscillatingpivot2(inter,ic,n,force,on_off,makemovie) %on_off 1=turn on movie 0=turn off

%--------------------------------------------------------------------------
%                            Make QT file
%--------------------------------------------------------------------------

MakeQTMovie('start','pro7unstabledown.mov')
...
for k=1:n
  ...
  xrod = [0 xbob]; yrod = [0 ybob];
    ...
    if on_off==1
        ...
        drawnow; % pause(h/100)   
        title(sprintf('Oscillating Pivot A= %d, y= %d',force,ic(1)))
    end

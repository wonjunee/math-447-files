...
% Example usage: w=poisson5(0,4,0,4,10,10,5,3.85,0) 
function [w, x, y]=poisson5(xl,xr,yb,yt,M,N,P,K,size) %'size' is the size of power input at the left
...
for j=2:2+size % left boundary points input
  i=1;  % Left
  A(i+(j-1)*m,i+(j-1)*m)=-3/(2*h);
  A(i+(j-1)*m,i+1+(j-1)*m)=2/h;
  A(i+(j-1)*m,i+2+(j-1)*m)=-1/(2*h);
